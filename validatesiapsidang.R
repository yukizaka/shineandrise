...
vild <- read.csv('C:/Users/Florecita/Documents/SKRIPSI/fixdatasetV5.csv')
glimpse(vild)
vild$id = as.factor(vild$id)
str(vild$id)

vild <- read.transactions('C:/Users/Florecita/Documents/SKRIPSI/fixdatasetV5.csv', format = 'basket', sep=',')
...
write.csv(train2, "uji_train5.csv", row.names = FALSE)
write.csv(test2, "uji_test5.csv", row.names = FALSE)

train_data2 <- read.transactions('C:/Users/Florecita/Documents/SKRIPSI/ProcessingData/shineandrise/uji_train5.csv', format = 'basket', sep=',')
test_data2 <- read.transactions('C:/Users/Florecita/Documents/SKRIPSI/ProcessingData/shineandrise/uji_test5.csv', format = 'basket', sep=',')
...
##############################################################################################
vild$Id <- as.factor(vild$id)
vild$JenisKelamin <- factor(vild$JenisKelamin, levels = c('Lk','Pr'))
vild$Ipk <- factor(vild$Ipk, levels = c('P','SM', 'M', 'TP'))
vild$LamaStudi <- factor(vild$LamaStudi, levels = c('A', 'B', 'C', 'D', 'E'))
vild$JalurMasuk <- factor(vild$JalurMasuk, levels = c('SNMPTN', 'SBMPTN', 'SPMU'))
vild$Prodi <- factor(vild$Prodi, levels = c('Informatika', 'Teknik Sipil', 'Teknik Elektro', 'Teknik Mesin'))
vild$AsalSekolah <- factor(vild$AsalSekolah, levels = c('A1', 'A2', 'A3'))
